...
export default function OptionalInfoScreen() {
  ...
  return (
    <AppScreen navigation contentStyle={styles.content}>
      ...
      <Pressable onPress={() => setOpen(!open)} style={styles.select}><Text style={styles.selectText}>{sleepHours || '0'}</Text><View style={styles.selectValue}><Text style={styles.selectText}>시간</Text><Icon name="chevronDown" size={14} color={colors.textTertiary} /></View></Pressable>
...
const styles = StyleSheet.create({ content: { gap: spacing.md }, previous: { ...typography.h1, color: colors.textTertiary }, title: { ...typography.subtitle, color: colors.text }, description: { ...typography.body, color: colors.textMuted }, label: { ...typography.label, color: colors.text, marginTop: spacing.sm }, requiredMark: { color: colors.danger }, summaryCard: { flexDirection: 'row', gap: spacing.md, padding: spacing.md, borderRadius: radius.lg, backgroundColor: '#DFE7E5' }, summaryText: { flex: 1, ...typography.label, color: colors.textTertiary }, select: { height: 54, borderRadius: radius.md, paddingHorizontal: spacing.md, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.surface }, selectValue: { flexDirection: 'row', alignItems: 'center', gap: 4 }, selectText: { ...typography.body, color: colors.textTertiary }, options: { maxHeight: 220, borderRadius: radius.md, backgroundColor: colors.surface, overflow: 'hidden' }, option: { padding: 12 }, optionText: { ...typography.body, color: colors.textTertiary }, grid: { flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: -5, padding: 10, borderRadius: radius.lg, backgroundColor: '#DFE7E5' }, half: { width: '50%', padding: 5 }, scanNote: { ...typography.caption, color: colors.textTertiary }, errorText: { ...typography.caption, color: colors.danger } });
