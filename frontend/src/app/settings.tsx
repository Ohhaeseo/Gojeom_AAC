...
export default function SettingsScreen() {
  ...
  return (
    <AppScreen navigation contentStyle={styles.content}>
      ...
      <GoModal visible={exit} title="로그아웃할까요?" description="계정에 저장된 정보는 유지되며 다시 로그인하면 이어서 사용할 수 있어요." confirmLabel="로그아웃" onClose={() => setExit(false)} onConfirm={() => { logout(); setExit(false); router.replace('/auth'); }} />
      <GoModal visible={remove} destructive title="정보를 삭제할까요?" description="계정과 프로필 사진, 분석 결과, 루틴 정보가 모두 삭제되며 같은 아이디로 로그인할 수 없어요." confirmLabel="삭제하기" onClose={() => setRemove(false)} onConfirm={() => { deleteAccount(); setRemove(false); router.replace('/auth'); }} />
    </AppScreen>
  );
}
