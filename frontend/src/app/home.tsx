      ...
      {hasProfile ? (
        <>
          ...
          <View style={styles.progressHead}>
            ...
            {/*
              **링이 무엇을 세는지 고른다.** 목표 카드 Pressable 밖이라 눌러도
              루틴 화면으로 넘어가지 않는다. (N-11)
            */}
          ...
          */}
          ...
          {tasks.length ? <Pressable onPress={() => router.push('/routines')} style={styles.progressCard}><GoalProgressRing completed={scope === 'TODAY' ? todayDone : done} total={scope === 'TODAY' ? today.length : tasks.length} /><Text style={styles.progressText}>{ordered.slice(0, 3).map((task) => `${task.status === 'DONE' ? '✓ ' : ''}${task.title}`).join('\n')}</Text></Pressable> : <Pressable accessibilityRole="button" accessibilityLabel="목표 만들러 가기" onPress={() => router.push('/routines')} style={styles.emptyAnalysis}><Text style={styles.emptyTitle}>아직 설정한 목표가 없어요.</Text><Text style={styles.emptyText}>진단 결과로 만들거나, 진단 없이 바로 만들 수도 있어요.</Text></Pressable>}
