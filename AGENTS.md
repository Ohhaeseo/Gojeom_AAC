...
### 구독은 만들되 결제는 붙이지 않는다 〔2026-08-19 결정 · 08-14 결정을 대체〕

> **2026-08-14에는 "구독을 구현하지 않는다"였다.** 두 번째 분석이 402로 막히는데
> 사용자에게 아무 출구가 없다는 것이 발표에서 문제가 되어, 8/19에 **구독 흐름만
> 열기로 바꿨다.** 결제(PG·웹훅·`payments`)는 여전히 범위 밖이다.

| 항목 | 상태 |
| --- | --- |
| `GET /subscriptions/me` | ✅ 구현 |
| `POST /subscriptions/subscribe` | ✅ 구현 — **결제 없이 즉시 활성화** |
| `POST /subscriptions/checkout` · `/webhook` | **구현하지 않음.** 결제가 없어 "결제 시작"이라 부를 것이 없다 |
| `payments` 테이블 | 스키마만 존재. 코드 없음 |
| 유료 구독 중 분석 무제한 | ✅ 구현 (`SubscriptionPlan.isUnlimited()`) |
| **구독 만료 시 차단** (PRD F-12) | ✅ `canAnalyze`가 `expiresAt`을 본다 |
| 가입 시 `TRIAL` + 분석권 1회 | 유지 |
| 요금제 화면 (`/plan`) · 소진 시 구독 모달 | ✅ 구현 |

**🔴 목업이 아니다.** `구독하기`를 누르면 `subscriptions` 행의 `plan`·`expires_at`이
실제로 바뀌고, 이후 분석 허용도 그 상태를 읽는다. 규칙 15를 지키려고 이렇게 했다 —
"구독된 척"하는 화면 상태를 만들지 않는다.

**분석권(`analysis_credits`)은 무료 체험 1회를 세는 값이다.** 유료 구간에서는 세지
않는다. `consumeCredit`이 `CASE`로 TRIAL일 때만 줄이는 이유이고, 이걸 안 하면
8,900원을 내고도 한 번만 되는 꼴이 된다.

> 결제가 실제로 붙을 때 — `/subscribe`는 없애고 `/checkout` → PG → `/webhook`에서
> `subscribe()`를 부르게 옮긴다. 엔티티의 `subscribe()`는 그대로 쓸 수 있다.
