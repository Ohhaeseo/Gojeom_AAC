...
> ⚠️ **`imageStatus`는 참고 사진 유무와 무관하게 항상 `SKIPPED`다.** 이미지 생성 단계가 이번 범위 밖이기 때문이다. `PENDING`으로 두면 영영 끝나지 않는 폴링이 되고, `FAILED`로 두면 시도조차 안 한 것을 실패라고 알리는 셈이다. 프론트는 `SKIPPED`에서 슬라이더를 렌더링하지 않으므로 화면이 성립한다. 이미지 생성을 붙일 때 `AnalysisPipeline`의 `IMAGE_STATUS_UNTIL_IMAGE_STAGE_EXISTS` 상수를 지우고 참고 사진 유무로 분기한다.
...
| 항목 | 왜 |
| --- | --- |
| `IMAGE_GENERATION` (비교 이미지) | 이번 스프린트 범위 밖. `gpt-image-1` 미검증 (§0-1). `imageStatus`는 `SKIPPED` 고정 |
...
| **비교 이미지 생성** (`IMAGE_GENERATION`) | 스프린트 범위 밖. `imageStatus`는 `SKIPPED` 고정 |
